...
import { provideServerContext } from '../composables/useServerContext'
...
const goToConsole  = () => router.push({ name: 'ServerConsole',  params: { id: serverId.value } })
const goToFiles    = () => router.push({ name: 'ServerFiles',    params: { id: serverId.value } })
const goToSettings = () => router.push({ name: 'ServerSettings', params: { id: serverId.value } })
const goToMods     = () => router.push({ name: 'ServerMods',     params: { id: serverId.value } })
...
const loadServer = async (options = {}) => {
  ...
  try {
    ...
    if (!serverSettings.value || route.name !== 'ServerSettings') {
...
watch(() => route.name, (name) => {
  ...
  if (name === 'ServerFiles' && !fileBrowser.value.entries.length && !fileBrowser.value.loading) {
    openFileBrowser()
  }
})
...
const startLogPolling = () => {
  if (logsIntervalId || route.name !== 'ServerConsole') {
    return
  }
  ...
  <div class="server-layout">
    ...
    <div class="server-layout__main">
      ...
      <main class="server-layout__content">
        <div v-if="serverLoading && !server" class="server-layout__loading">Loading server data…</div>
        <div v-else-if="!server" class="server-layout__loading">Unable to find this server.</div>
        <router-view v-else />
      </main>
    </div>
