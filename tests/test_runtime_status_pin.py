...
Frontend (stores/server.js:RUNTIME_KNOWN_STATUSES, ServerSwitcher.vue) mirrors
this set. When the backend set changes, the frontend mirror MUST be updated
in lockstep — this test is the canary.

If you change `_RUNTIME_KNOWN_STATUSES` in `backend/server/routes.py`, you must:
1. Update this pin (see expected set below).
2. Update the frontend mirror in `frontend/src/stores/server.js:RUNTIME_KNOWN_STATUSES`.
3. Update any divergent frontend status-allowlists
   (`ServerSwitcher.vue`, `AppTopbar.vue`, `StatusPill.vue`).
4. Reference the new pin from CC5 in CONVENTIONS.md.
...
def test_runtime_known_statuses_is_pinned():
    ...
    assert _RUNTIME_KNOWN_STATUSES == expected, (
        ...
        f"frontend/src/stores/server.js:RUNTIME_KNOWN_STATUSES + this pin."
    )
